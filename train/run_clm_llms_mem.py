...
def main():
    ...
    if model_args.model_name_or_path:
        ...
        # zefeng: 2023-04-20, fix the bug of loading model from checkpoint with a max_length less than block size
        if model.config.max_position_embeddings < data_args.block_size:
            model.config.max_position_embeddings = data_args.block_size
    ...
    if data_args.block_size is None:
        ...
        # In fact, llama uses relative positions, its tokenizer's max_length is infinite, and the if statement actually won't execute.
